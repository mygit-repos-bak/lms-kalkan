/*
  # Legal Management Platform Database Schema
...
DROP POLICY IF EXISTS "Users can read items based on role" ON items;
DROP POLICY IF EXISTS "Users can create items based on role" ON items;
DROP POLICY IF EXISTS "Users can update items based on role" ON items;
DROP POLICY IF EXISTS "Users can read tasks based on item access" ON tasks;
DROP POLICY IF EXISTS "Users can manage tasks based on role" ON tasks;
DROP POLICY IF EXISTS "Legal meta access follows item access" ON legal_meta;
DROP POLICY IF EXISTS "Deal meta access follows item access" ON deal_meta;
DROP POLICY IF EXISTS "Real estate meta access follows item access" ON real_estate_meta;
DROP POLICY IF EXISTS "Anyone can read tags" ON tags;
DROP POLICY IF EXISTS "Admins can manage tags" ON tags;
DROP POLICY IF EXISTS "Anyone can read stages" ON stages;
DROP POLICY IF EXISTS "Admins can manage stages" ON stages;
DROP POLICY IF EXISTS "Users can read comments on accessible items/tasks" ON comments;
DROP POLICY IF EXISTS "Users can create comments on accessible items/tasks" ON comments;
...
DROP POLICY IF EXISTS "Junction tables follow parent access" ON item_assignees;
DROP POLICY IF EXISTS "Junction tables follow parent access" ON item_tags;
DROP POLICY IF EXISTS "Junction tables follow parent access" ON task_assignees;
DROP POLICY IF EXISTS "Junction tables follow parent access" ON task_tags;

-- RLS Policies

-- Users policies
CREATE POLICY "Users can read all users" ON users FOR SELECT TO authenticated USING (true);
CREATE POLICY "Users can update own profile" ON users FOR UPDATE TO authenticated USING (auth.uid() = users.id);
...
CREATE POLICY "System can create activity logs" ON activity_logs FOR INSERT TO authenticated WITH CHECK (true);
